...
\begin{figure}
	...
	\begin{subfigure}[b]{.48\textwidth}
		\begin{tikzpicture}
			...
			(p2) edge[post] node[above] {1} (t2);
		\end{tikzpicture}
		\subcaption{Abwandlung des Petrinetzes aus Abbildung~\ref{fig:petrinet}. Die Gewichtungsfunktion ist für jede Relation maximal 1.}\label{fig:1-bpetrinet:graph}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{.48\textwidth}
		\begin{tikzpicture}[node distance=5mm,auto,state/.append style={rounded rectangle}]
	...
	\end{subfigure}
\caption{Petrinetz und zugehöriger Erreichbarkeitsgraph, in dem die Anzahl der Markierungen pro Platz stets maximal 1 ist. }\label{fig:1-bpetrinet}
\end{figure} 
