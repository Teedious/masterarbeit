\begin{figure}
	...
	\end{tikzpicture} 
	\caption[Bildschirmfoto der Blockformation in Szenario 2: Halbwürfel.]{Bildschirmfoto der Blockformation in Szenario 2: Halbwürfel. Es wird das Gitternetz der Blöcke dargestellt, um die Menge der angezeigten Elemente zu maximieren. Zur besseren Ansicht sind zwei Bereiche vergrößert dargestellt. Links unten ist die Zerteilung der Würfelseiten in je zwei Dreiecke zu erkennen. Links oben wird ersichtlich, dass das Gitternetz tatsächlich nur die Kanten der Dreiecke zeichnet.}\label{fig:cube}
\end{figure}
...
\paragraph{\ac{fps}}Der Verlauf der Framerate für Szenario 2 ist in Abbildung~\vref{fig:seed-0-cube-fps} dargestellt.
In \sysA{} werden Frames ab Sekunde $11$ erzeugt, in \sysB{} ab Sekunde $13$. Die mittlere Framerate in \sysA{} ist \SI{253}{\fps} und in \sysB{} \SI{417}{\fps}. Damit erhöht sich die Framerate in \sysB{} um \SI{65}{\percent}. Dieser Wert ist deutlich geringer als in Szenario 1. Da die Bildwiederholrate in beiden Systemen stark gesunken ist, könnte das daran liegen, das die \ac{gpu} stärker ausgelastet ist. Die Frameraten bleiben in beiden Systemen wieder annähernd konstant.
\begin{figure}[!htb]
	\fpsplot{seed-0-cube}
	\caption{Graph des Verlaufs der Framerate in Szenario 2: Halbwürfel.}\label{fig:seed-0-cube-fps}
\end{figure}

\paragraph{\ac{cpu}} Die Last der \ac{cpu} in Szenario 2, zu sehen in Abbildung~\vref{fig:seed-0-cube-cpu}, gestaltet sich vergleichbar zu der in Szenario 1. Die Mittelwerte sind mit \SI{13}{\percent} in \sysA{} und \SI{18}{\percent} in \sysB{} nach Rundung identisch zu Szenario 1.
\begin{figure}[!htb]
	\cpuplot{seed-0-cube}
...
\begin{figure}[!htb]
	\memplot[xticklabels={,,},xlabel={}]{seed-0-cube-multi-mem.csv}{\sysA{}}
