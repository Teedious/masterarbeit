...
\begin{figure}
	\includesvg[width=.8\textwidth]{Loader.svg}
...
Die Klasse \class{Player} ist beispielsweise für die Simulation des Spielercharakters zuständig. Zur Veranschaulichung ist ein Screenshot der Spielerfigur in Abbildung~\vref{fig:player} dargestellt. Wird ein neuer Spielercharakter erstellt, so ruft die Klasse \class{Player} die Methode \code(init(): void) auf, wodurch über verschiedene Methodenaufrufe zum Schluss auch \code{Loader.loadToVAO(...)} aufgerufen wird. Damit steht das zu ladende Spielercharakter-Modell ab dem nächsten Frame zur Verfügung und kann genutzt werden, um den Charakter zu zeichnen. Wie der Vorgang des Zeichnens aufgebaut ist, wird in den folgenden Abschnitten erklärt. Dort wird dieses Beispiel fortgeführt.
...
\begin{figure}
	\centering
	\includesvg[width=.8\textwidth]{Rendering-Interface.svg}
...
\begin{figure}
	...
	\begin{tikzpicture}[scale=.9]
		...
		\end{tikzpicture}
		\caption{Wechsel der Puffer in \class{MasterRenderer} während \code{swapBuffer()}. In Schritt 1 wird der Inhalt von \var{lastUsedRenderables} in die Liste den Cache verschoben. In Schritt 2 werden die Listen von \var{usedRenderables} und \var{lastUsedRenderables} vertauscht.}\label{fig:mrSwapBuffer}
\end{figure}
...
Die \ac{gui}-Elemente werden dagegen von der Klasse \class{UIRenderer} verwaltet und müssen nicht jeden Frame zu einer Menge von zu zeichnenden Elementen hinzugefügt werden.
