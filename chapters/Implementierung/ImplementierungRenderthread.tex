...
\end{figure}
Die \classGameLoop{}-Klasse ruft in der \gls{Aktivitaet} \enquote{Tausche Puffer} die Methode \code{swapBuffer()}, in \enquote{Lade Daten auf \ac{gpu}} die \code{loadGraphics()}-Methode und in \enquote{Rendering} \code{render()} auf. Wie in der Abbildung durch die schwarzen Nebenläufigkeits-Balken zu erkennen ist, werden die Simulation und das Rendering in \code{stepWindow()} nebenläufig ausgeführt. Die restlichen \glspl{Anweisung} werden sequentiell ausgeführt, wie in den vorherigen Abschnitten beschrieben.