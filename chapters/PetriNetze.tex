...
\subsection{Transitionsregel} Um nun das Verhalten von Programmen beschreiben zu können, kann die Markierung eines Petrinetzes anhand der folgenden sogenannten Transitions-Regel geändert werden~\cite{Murata1989}.
\begin{enumerate}
...
\begin{enumerate}
	\item Die Menge der Knoten mit Kanten zu einem Knoten $k \in P\cup T$ heißt \emph{Vorbereich} $^\circ k = \{v | (v,k) \in F\}$ des Knotens $k$.
	\item Die Menge der Knoten mit Kanten von einem Knoten $k \in P\cup T$ heißt \emph{Nachbereic} $k^\circ  = \{n | (k,n) \in F\}$ des Knotens $k$.
\end{enumerate}



\subsection{Erweiterte Petri-Netze}
