...
Um formal Eigenschaften eines Programms zu finden, ist es möglich dieses mittels eines Petri-Netzes (automatisiert) zu modellieren. Dazu müssen Petri-Netz-Konstrukte genutzt werden, die die Anweisungen des Programms abbilden können. Für die Analyse nebenläufiger Programme sind nur Anweisungen \emph{Rendevous}-Anweisungen sowie Kontrollstrukturen relevant~\cite{Goel1990}. \todo{weiterschreiben}
...
Formal können Wettkampfbedinungen auch mittels erweiterten Petri-Netzen definiert werden. Eine Markierung eines erweiterten Petri-Netzes enthält eine Wettkampfbedingung, wenn sich schreibende und lesende Variablen in der Markierung überschneiden. Dazu muss man die Transitionen in den Vorbereichen der markierten Plätze betrachten also $ {}^\circ\!M_{\geq1} \coloneqq \bigcup_{p \in M_{\geq1}} {}^\circ p$. Wie in Abschnitt~\ref{sec:petri} definiert, ist $M_{\geq1}$ die Menge der Knoten, die mindestens eine Marke enthalten. Eine Wettkampfbedingung lässt sich nun feststellen, wenn die paarweisen Schnitte der Variablenmengen nicht leer sind:
\begin{align*}
	\text{Wettkampfbedingung} \iff \exists s,t \in {}^\circ\!M_{\geq1}, s \neq t: S(s) \cap (S(t) \cup L(t)) \neq \emptyset.
\end{align*}
Die Menge der \emph{kritischen} Variablen, die zur Existenz der Wettkampfbedingung fürhen, ist gegeben durch
\begin{align*}
	V_\text{krit} = \bigcup_{\substack{s,t\, \in \,{}^\circ\!M_{\geq1}\\s\neq t}} \left( S(s) \cap (S(t) \cup L(t))\right).
\end{align*}
Es lässt sich leicht zeigen, dass gilt $V_\text{krit} \neq \emptyset \iff \text{Wettkampfbedingung}$. Der Beweis wird hier der Übersichtlichkeit halber ausgelassen. 
...
\begin{figure}
	...
	\begin{tikzpicture}[node distance=2cm,on grid, auto]
		...
		\node[place, label=$p_5$, tokens=1, right = of t3] (p5) {};
		...
		;
	\end{tikzpicture}
	\caption{Ein erweitertes Petri-Netz, dessen Markierung eine Wettkampfbedingung enthält. Die Schreibmenge von $p_3$ enthält die Variable $b$, diese ist allerdings auch in der Lesemenge von $p_2$ enthalten. Die problematische Variable $b$ ist rot markiert. Das Auftauchen von $b$ in der Lesemenge von $p_3$ stellt kein Problem dar.}\label{fig:wettkampfpetri}
\end{figure}

Um das Konzept zu veranschaulichen, ist in Abbildung~\ref{fig:wettkampfpetri} ein erweitertes Petri-Netz gezeigt, dessen Markierung eine Wettkampfbedingung enthält. Die Plätze $p_4$ und $p_5$ enthalten je eine Markierung. Daher müssen nun ihre Vorbereiche betrachtet werden. Diese sind $\{t_2\}$ und $\{t_3\}$. Wie in der Abbildung rot markiert, ist ersichtlich, dass sich die Lese- und Schreibmenge der Vorbereiche überschneiden, da $V_\text{krit} = \{b\} \neq \emptyset$ ist. Somit enthält die gezeigte Markierung eine Wettkampfbedingung.
