...
\TODO{Reccource/Datum definieren*}{Daten} und spielen in Programmen eine zentrale Rolle. Greift eine Gruppe von Anweisungen auf die selben (geteilten) Daten zu, ist die Reihenfolge des Zugriffs, wie alle anderen Aktivitäten, beliebig. Wenn mindestens eine der Anweisungen schreibend auf die geteilten Daten zugreift (diese also verändert), hängt der Zustand der Ausgabe des Programms im Allgemeinen von der Reihenfolge der Ausführung ab. Diese Situation wird \emph{Wettkampfbedinung} (engl. Race Condition) genannt~\cite{Hettel2016}. Erwähnenswert ist hier, dass Wettkampfbedingungen nur auftreten können, wenn die geteilten Daten nebenläufig geändert werden. Ausschließlich lesende Zugriffe sind unkritisch, da die Daten unabhängig von der Ausführreihenfolge immer identisch sind. Wenn Daten von lesenden Anweisungen zu jederzeit in einem Zustand gefunden werden, der korrekt ist, werden sie als \emph{threadsicher} bezeichnet. Formal können Wettkampfbedinungen auch mittels erweiterten Petri-Netzen definiert werden. Eine Markierung eines erweiterten Petri-Netzes enthält eine Wettkampfbedingung, wenn sich schreibende und lesende Variablen in den Vorbereichen der Plätze mit mindestens einer Marke: 
\begin{align*}
	\emptyset \neq \bigcup_{p \in M_{\geq1}} \left( S(p) \cap \left(\bigcup_{q \in M_{\geq1}\setminus \{p\}} S(q) \cup L(q) \right)\right).
\end{align*}
$M_{\geq1}$ ist, wie in Abschnitt~\ref{sec:petri} definiert, die Menge der Knoten, die mindestens eine Marke enthalten. Um das Konzept zu veranschaulichen, ist in Abbildung~\ref{fig:wettkampfpetri} ein erweitertes Petri-Netz gezeigt, dessen Markierung eine Wettkampfbedingung enthält.
\begin{figure}
	\centering
	\begin{tikzpicture}[node distance=2cm,on grid, auto]
		\node[place, label=$p_1$] (p1) {};
		\node[transV, label=$t_1$, right = of p1] (t1) {};
		\node[place, tokens=1, label=$p_2$,label=right:{\LSset{a,{\color{red}b}}{a}}, above right = of t1] (p2) {};
		\node[place, tokens=1, label=$p_3$,label=right:{\LSset{b}{{\color{red}b}}}, below right = of t1] (p3) {};
		\node[transV,label=$t_2$, below right = of p2] (t2){};
		...
		(p2) edge[post] node [swap] {1} (t2)
		(p3) edge[post] node  {1} (t2)
